...
# 環境変数（DATABASE_URL / DATABASE_URL_UNPOOLED に接続文字列を設定）
cp .env.example .env
...
2. Storage タブから Postgres（Neon 等）を追加し、プロジェクトに接続する
   - 接続すると `DATABASE_URL`（アプリ用・プーラー経由）と `DATABASE_URL_UNPOOLED`（マイグレーション用・直接接続）が自動でプロジェクトの環境変数に設定される。変数名が異なる場合は `prisma/schema.prisma` の `env()` 参照名を合わせる
